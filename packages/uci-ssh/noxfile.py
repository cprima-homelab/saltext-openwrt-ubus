...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/uci_ssh/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/uci_ssh/*")
            ...
            #    "--include=src/saltext/uci_ssh/*,tests/*"
            # )
        finally:
