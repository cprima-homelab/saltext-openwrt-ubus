...
Usage::
    ...
    python -m saltext.uci_ubus.data_manifest           # YAML to stdout
    python -m saltext.uci_ubus.data_manifest --json    # JSON to stdout
    python -m saltext.uci_ubus.data_manifest --check   # path analysis
...
from saltext.uci_ubus.sensitivity.model import Classification, SensitivityProfile, Surface

_ANNOTATED_MODULES = [
    "saltext.uci_ubus.sensitivity.classify",
    "saltext.uci_ubus.sensitivity.project",
    ...
    for c in crossings:
        ...
        if result == "safe":
            if guards and guards != [fn_name]:
                reason = f"{from_b} → {to_b}; guarded by {guards}"
            else:
            ...
            results.append(
                (
                    "WARN",
                    fn_name,
                    f"{from_b} → {to_b}; handles_secrets or emits_secrets is null — path is uncharted",
                )
            )
        else:
            results.append(
                (
                    "FAIL",
                    fn_name,
                    f"{from_b} → {to_b}; secrets reach external boundary with emits_secrets=true",
                )
            )
