...
def init(opts):
    ...
    for key in ("host", "password"):
        if key not in proxy_conf:
            raise ValueError(f"uci_ubus_jsonrpc: required pillar key '{key}' is missing")
    ...
    if client.session_timeout < session_timeout:
        log.warning(
            "rpcd granted session timeout of %ds (requested %ds). Staged UCI changes may be lost between state runs.",
    ...
    log.info("uci_ubus_jsonrpc proxy initialized for %s", proxy_conf["host"])
...
def shutdown(opts):  # pylint: disable=unused-argument
    ...
    DETAILS.clear()
    log.info("uci_ubus_jsonrpc proxy shut down")
