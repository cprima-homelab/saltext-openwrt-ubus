...
from saltext.uci_ubus._internal import scope
from saltext.uci_ubus._internal.ubus_ops import diff_section as _diff_section
from saltext.uci_ubus._internal.ubus_ops import resolve_sections as _resolve_sections
        ...
        for section_name, section_changes in updates.items():
            ...
            if section_name not in current:
                ...
                if not type_:
                    ret["result"] = False
                    ret["comment"] = (
                        f"Section '{section_name}' does not exist in {config} and no _type specified for creation"
                    )
                    return
