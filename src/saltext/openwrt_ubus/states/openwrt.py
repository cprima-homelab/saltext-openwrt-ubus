"""
Shorthand alias for the ``openwrt_ubus`` state module.
...
def managed(name, config, sections, apply_rollback=None, revert_pending=False):
    """
    Ensure named UCI sections match desired state. Alias for ``openwrt_ubus.managed``.
          ...
          openwrt.managed:
            ...
            - sections:
                lan:
                  ...
                  ipaddr: 10.35.24.1
    """
    return __states__["openwrt_ubus.managed"](
...
def applied(name, config=None, rollback=None):
    """
    Apply staged UCI changes with rollback protection. Alias for ``openwrt_ubus.applied``.
    ...
    .. code-block:: yaml
        ...
        apply_staged:
          openwrt.applied: []
    """
    return __states__["openwrt_ubus.applied"](name, config=config, rollback=rollback)
