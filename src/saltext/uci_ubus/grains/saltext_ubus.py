"""
Grains module for OpenWrt devices managed via uci_ubus proxies.
...
Supports both ``uci_ubus_jsonrpc`` and ``uci_ubus_ssh`` proxy types.
...
_SUPPORTED_PROXYTYPES = frozenset({"uci_ubus_jsonrpc", "uci_ubus_ssh"})
...
def __virtual__():
    ...
    if __opts__.get("proxy", {}).get("proxytype") not in _SUPPORTED_PROXYTYPES:
        return False, "proxytype is not uci_ubus_jsonrpc or uci_ubus_ssh"
