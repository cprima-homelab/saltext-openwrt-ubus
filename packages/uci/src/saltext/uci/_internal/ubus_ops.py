    ...
    for type_, sections_list in anon_by_type.items():
        ...
        if len(stripped) == 1:
            ...
            if match_key is not None:
                ...
                entry["_comment"] = (
                    f"No unique match key detected for type '{type_}'. Set _match manually before using this pillar."
                )
...
def config_evidence(
    call, config, source_device, collector, transport, collector_version, profile=None
):
    """
    ...
    Schema::
        ...
        provenance:
          collector:         <installed transport package, e.g. saltext-uci-ubus>
    ...
    Reusable across transport modules — each passes its own ``collector`` and
    ``transport`` labels (this function lives in core and has no way to know
    which transport package is actually calling it).
    """
    if profile is None:
        profile = SensitivityProfile.load_builtin()
...
def runtime_evidence(call, domain, source_device, collector, transport, collector_version):
    """
    ...
    No storage side effect. ``source_type`` names the device platform
    (constant regardless of transport) — it is not the same thing as
