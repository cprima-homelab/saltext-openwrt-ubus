...
Requests go through ``client._opener`` (a ``urllib.request.OpenerDirector``
built in ``__init__`` for SNI-override/redirect-handling support), not the
bare ``urllib.request.urlopen`` function -- so tests patch
``client._opener.open`` directly rather than the module-level function.
"""
...
from saltext.uci_ubus._internal.rpc import (
    NULL_SESSION,
    UBUS_STATUS_OK,
...
class TestClientInit:
    def test_url_construction(self):
        client = UbusRpcClient("10.0.0.1", "user", "pass", scheme="https", port=443)
        assert client.url == "https://10.0.0.1:443/ubus"

    def test_custom_port(self):
        client = UbusRpcClient("10.0.0.1", "user", "pass", scheme="https", port=8443)
        assert client.url == "https://10.0.0.1:8443/ubus"

    def test_url_unset_without_explicit_scheme_and_port(self):
        """Without both scheme and port, discovery decides the URL later."""
...
class TestLogin:
    def _make_client(self, **kwargs):
        return UbusRpcClient("10.0.0.1", "salt", "pass", scheme="https", port=443, **kwargs)

    def test_successful_login(self):
        client = self._make_client()
        body = _jsonrpc_response(result=[UBUS_STATUS_OK, {"ubus_rpc_session": "abc123def456", "timeout": 300}])
        with patch.object(client._opener, "open", return_value=_mock_urlopen(body)):
            token = client.login()
    ...
    def test_login_sends_null_session(self):
        client = self._make_client()
        body = _jsonrpc_response(result=[UBUS_STATUS_OK, {"ubus_rpc_session": "tok", "timeout": 300}])
        with patch.object(client._opener, "open", return_value=_mock_urlopen(body)) as mock_open:
            client.login()
    ...
    def test_login_passes_session_timeout(self):
        ...
        client = self._make_client(session_timeout=600)
        body = _jsonrpc_response(result=[UBUS_STATUS_OK, {"ubus_rpc_session": "tok", "timeout": 600}])
        with patch.object(client._opener, "open", return_value=_mock_urlopen(body)) as mock_open:
            client.login()
...
class TestCall:
    def _make_client(self):
        client = UbusRpcClient("10.0.0.1", "salt", "pass", scheme="https", port=443)
    ...
    def test_auto_relogin_on_no_session(self):
        ...
        login_body = _jsonrpc_response(result=[UBUS_STATUS_OK, {"ubus_rpc_session": "new_tok", "timeout": 300}])
        ...
        with patch.object(client._opener, "open", side_effect=lambda *a, **kw: next(responses)):
            result = client.call("uci", "configs")
        assert result == {"configs": ["network"]}
